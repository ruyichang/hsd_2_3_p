...
class RPC extends RPCBase {
  ...
  async _handleWork(data) {
    ...
    if (data.length !== 256) //256 +3 + 3
      return [false, 'invalid-data-length'];

    let num_txs =  data.substr(0,3);
    this.logger.debug('num_txs     : %d', num_txs);

    let data_ = data.substr(4,256);

    const hdr = Headers.fromMiner(data_);
  ...
  async _createWork() {
    ...
    return {
      ...
      fee: attempt.fees,
      num_txs: attempt.num_txs,
      num_wts: attempt.num_wts,
      num_s: attempt.items.length
    };
  }
  ...
  async submitWork(args, help) {
    ...
    this.logger.warning('---valid: %s',
        valid);
    this.logger.warning('---valid: %x',
        valid.toString('hex'));

    const data = valid.buf(0);
    const mun_txs_ = valid.buf(1);
