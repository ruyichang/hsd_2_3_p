...
class RPC extends RPCBase {
  ...
  async handleWork(data, seedkey_) {
    const unlock = await this.locker.lock();
    try {
      return await this._handleWork(data, seedkey_);
    } finally {
      unlock();
    }
  }

  async _handleWork(data, seedkey_) {
    // const attempt = this.attempt;
    this.logger.debug('---this attempt.items.length: %d', this.attempt.items.length);

    const attempt = this.attemptMap.get(seedkey_);
    ...
    this.logger.debug('attempt.items.length: %d', attempt.items.length);

    const hdr = Headers.fromMiner(data);

    this.logger.debug('---------------------------------------------------------');
    this.logger.debug('_handleWork hdr.version     : %d', hdr.version);
    this.logger.debug('_handleWork hdr.prevBlock   : %x', hdr.prevBlock);
    this.logger.debug('_handleWork hdr.merkleRoot  : %x', hdr.merkleRoot);
    this.logger.debug('_handleWork hdr.witnessRoot : %x', hdr.witnessRoot);
    this.logger.debug('_handleWork hdr.treeRoot    : %x', hdr.treeRoot);
    this.logger.debug('_handleWork hdr.reservedRoot: %x', hdr.reservedRoot);
    this.logger.debug('_handleWork hdr.time        : %d', hdr.time);
    this.logger.debug('_handleWork hdr.bits        : %d', hdr.bits);
    this.logger.debug('_handleWork hdr.nonce       : %d', hdr.nonce);
    this.logger.debug('_handleWork hdr.extraNonce  : %x', hdr.extraNonce);
    this.logger.debug('_handleWork hdr.mask        : %x', hdr.mask);       // 00000XXXX...//I set it to all 0
    this.logger.debug('_handleWork hdr._maskHash   : %x', hdr._maskHash);  // black2b with previous hash
    this.logger.debug('_handleWork hdr._hash       : %x', hdr._hash);
    this.logger.debug('---------------------------------------------------------');
    ...
    this.logger.debug('_handleWork block.hash()       : %x', block.hash());
  ...
  async _createWork() {
    const [mask, attempt, seedkey] = await this.updateWork();
    const time = attempt.time;
    const data = attempt.getHeader(0, time, consensus.ZERO_NONCE, mask);

    this.attemptMap.set(seedkey.toString(), attempt);
    const aa = this.attemptMap.get(seedkey.toString());
    this.logger.warning('_createWork---attemptMap: %d', aa.height);
    this.logger.warning('_createWork---seedkey: %s', seedkey.toString('hex'));
    ...
    return {
      ...
      mask: mask.toString('hex'), //[Changry] Add to pool's method
      fee: attempt.fees,
      num_txs: attempt.items.length,
      seedkey: seedkey.toString('hex') //for to find attempt will submit
    };
  }
  ...
  async submitWork(args, help) {
    ...
    this.logger.warning('---valid: %s', valid);

    const data = valid.buf(0);
    const seedkey_ = valid.buf(1);
    let seedkey = seedkey_.toString();

    this.logger.info('[submitWork] seedkey_:%x', seedkey_);

    if (!data)
      throw new RPCError(errs.TYPE_ERROR, 'Invalid work data.');

    return this.handleWork(data, seedkey);
  }
  ...
  async updateWork() {
    ...
    if (attempt) {
      ...
      let seedkey = attempt.randomAttemptMask();

      this.maskMap.set(maskHash, mask);

      return [mask, attempt, seedkey];
    }
    ...
    if (last_height != this.attempt.height){
      last_height = this.attempt.height;
      this.attemptMap.clear();
      this.logger.info('-------updateWork new height:%d,clear attmptMap.',this.attempt.height);
    }
