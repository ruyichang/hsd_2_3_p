...
class RPC extends RPCBase {
  ...
  async handleWork(data, mun_txs_) {
    const unlock = await this.locker.lock();
    try {
      return await this._handleWork(data, mun_txs_);
    } finally {
      unlock();
    }
  }

  async _handleWork(data, mun_txs_) {
  ...
  async submitWork(args, help) {
    ...
    const valid = new Validator(args);
    this.logger.warning('---valid: %s',
        valid);

    const data = valid.buf(0);
    const mun_txs_ = valid.i64(1);
    ...
    return this.handleWork(data, mun_txs_);
  }
