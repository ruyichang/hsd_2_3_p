...
class BlockTemplate {
  ...
  fromOptions(options) {
    ...
    //changry
    if (options.num_txs != null) {
      assert(typeof options.num_txs === 'number');
      this.num_txs = options.num_txs;
    }
    if (options.num_wts != null) {
      assert(typeof options.num_wts === 'number');
      this.num_wts = options.num_wts;
    }
  ...
  refresh() {
...

    {
      ...
      console.log('num_txs:%d. merkleRoot:%x', this.num_txs, this.merkleRoot.toString('hex'));
    }

    {
      ...
      console.log('num_wts:%d. witnessRoot:%x', this.num_wts, this.witnessRoot.toString('hex'));
    }
...
/*    const mask = random.randomBytes(32);
  ...
  commit(proof, mun_txs_) {
    ...
    console.log('ncommit mun_txs_:%d', mun_txs_);
