...
/*    const mask = random.randomBytes(32);
  ...
  commit(proof, mun_txs_) {
    ...
    //changry
    console.log('--ncommit this.merkleRoot  :%x', this.merkleRoot.toString('hex'));
    console.log('--ncommit this.witnessRoot :%x', this.witnessRoot.toString('hex'));

    //if mun_txs_ > this.items.length?
    let no_tx = 0;
    ...
    if (mun_txs_ <= this.items.length){
      ...
      console.log('-no_tx :%d', no_tx );
    }

    //changry
    this.merkleRoot  = block.merkleRoot = block.createMerkleRoot();
    this.witnessRoot = block.witnessRoot = block.createWitnessRoot();

    console.log('-A ncommit this.merkleRoot  :%x', this.merkleRoot.toString('hex'));
    console.log('-A ncommit this.witnessRoot :%x', this.witnessRoot.toString('hex'));

    //发现coinbase也有随机数和fee的变化，需要拿到之前的coinbase，然后重新计算merkleRoot&witnessRoot
